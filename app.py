...
app = Flask(__name__)
app.secret_key = 'dashboard_trigo_secret_key_2024'  # Chave secreta para sessões
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  ...
                  email TEXT,
                  full_name TEXT,
                  role TEXT DEFAULT 'pending', -- admin, user, pending
...
def create_default_admin():
    ...
    if not c.fetchone():
        ...
        c.execute("INSERT INTO users (username, password, email, full_name, role) VALUES (?, ?, ?, ?, ?)",
                 ('admin', hashed_password, 'admin@dashboard.com', 'Administrador', 'admin'))
        conn.commit()
...
def verify_user(username, password):
    ...
    c.execute("SELECT * FROM users WHERE username = ? AND password = ? AND is_active = 1 AND role != 'pending'", 
...
def register_user(username, password, email, full_name):
    """Registra novo usuário (status pending)"""
    conn = sqlite3.connect(DATABASE_FILE)
    c = conn.cursor()
    ...
    try:
        hashed_password = hash_password(password)
        c.execute("INSERT INTO users (username, password, email, full_name, role) VALUES (?, ?, ?, ?, ?)",
                 (username, hashed_password, email, full_name, 'pending'))
        conn.commit()
        user_id = c.lastrowid
...
def get_pending_users():
    ...
    conn = sqlite3.connect(DATABASE_FILE)
    c = conn.cursor()
    c.execute("SELECT id, username, email, full_name, created_at FROM users WHERE role = 'pending' ORDER BY created_at DESC")
    users = c.fetchall()
    conn.close()
    return users
...
def get_all_users():
    ...
    conn = sqlite3.connect(DATABASE_FILE)
    c = conn.cursor()
    c.execute("SELECT id, username, email, full_name, role, created_at, is_active FROM users ORDER BY role DESC, username")
    users = c.fetchall()
    conn.close()
    return users
...
def log_access(user_id, action):
    ...
    conn = sqlite3.connect(DATABASE_FILE)
    c = conn.cursor()
    c.execute("INSERT INTO access_logs (user_id, action) VALUES (?, ?)", (user_id, action))
    conn.commit()
    conn.close()
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not user or user[5] != 'admin':  # user[5] é a coluna role
            flash('Apenas administradores têm acesso a esta funcionalidade.', 'danger')
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Usuário ou senha incorretos, ou conta pendente de aprovação.', 'danger')
...
@admin_required
def admin_users():
    """Página de administração de usuários (apenas admin)"""
...
@app.route('/admin/toggle_user/<int:user_id>/<int:is_active>')
@admin_required
def toggle_user_route(user_id, is_active):
    """Ativa/desativa usuário"""
    update_user_status(user_id, is_active)
    status = "ativado" if is_active else "desativado"
    flash(f'Usuário {status} com sucesso.', 'info')
    return redirect(url_for('admin_users'))
...
def slideshow():
    ...
    config = load_config()
    return render_template('slideshow.html', config=config, session=session)
...
def slideshow_fullscreen():
    ...
    config = load_config()
    return render_template('slideshow_fullscreen.html', config=config, session=session)
...
@app.route('/register_cultivar', methods=['POST'])
@login_required
